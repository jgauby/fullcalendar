...
function ResourceEventRenderer() {
	...
	var renderCellOverlay = t.renderCellOverlay;
	...
	function sliceSegs(events, resource, visEventEnds, start, end) {
		var segs = [],
		...
		for (i=0; i<len; i++) {
			...
			if (event.resourceId == resource.id && eventEnd > start && eventStart < end) {
				if (eventStart < start) {
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			"<div class='fc-event-time' style='" + skinCss + "'>" +
		...
		eventElement.draggable({
			...
			start: function(ev, ui) {
				...
				hoverListener.start(function(cell, origCell) {
					...
					if (cell) {
						...
						if (!cell.row) {
							// on full-days
							renderCellOverlay(0, cell.col, 0, cell.col);
							resetElement();
						}else{
			...
			stop: function(ev, ui) {
				...
				if (revert) {
					...
					eventDrop(this, event, 0, minuteDelta, allDay, ev, ui, resource);
				}
			}
		});
	...
	function draggableSlotEvent(event, eventElement, timeElement) {
		...
		eventElement.draggable({
			...
			start: function(ev, ui) {
				...
				resource = prevResource = opt('resources')[origCell.col];
			...
			stop: function(ev, ui) {
				...
				clearOverlays();
				trigger('eventDragStop', eventElement, event, ev, ui, resource);

				if (isInBounds && (isAllDay || dayDelta || minuteDelta)) { // changed!
					eventDrop(this, event, 0, isAllDay ? 0 : minuteDelta, isAllDay, ev, ui, resource);
				}
...
/* Resource Event Segment Utilities
-----------------------------------------------------------------------------*/
